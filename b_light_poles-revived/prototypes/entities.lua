...
local straight_poles = settings.startup["use-straight-poles"].value

local light_pole_w = table.deepcopy(data.raw['electric-pole']['medium-electric-pole'])

local function get_pole_filename(color_variant)
	return "__b_light_poles-revived__/graphics/entities/hr_medium-electric-pole_" .. (straight_poles and "straight_" or "") .. color_variant .. ".png"
end

local function get_connection_points()
	if straight_poles then
		return {{
					 shadow = {
...
light_pole_w.pictures = -- графика
	{
		...
		{
		{
			filename = get_pole_filename('w'),
			priority = "extra-high",
			width = 84,
			height = 252,
			direction_count = straight_poles and 1 or 4,
...
light_pole_w.connection_points = get_connection_points()
...
light_pole_r.pictures.layers[1].filename = get_pole_filename('r')
...
light_pole_g.pictures.layers[1].filename = get_pole_filename('g')
...
light_pole_o.pictures.layers[1].filename = get_pole_filename('o')
...
light_pole_b.pictures.layers[1].filename = get_pole_filename('b')
